...
class Interpreter:
  ...
  def assign(self, body, instance):
    ...
    #print(f"Attempting to save {value_to_store} to {variable_to_save_to}.")
    ...
    if body[-1] in self.reserved_words:
      ...
      # elif not body[0].startswith("\"") or not body[0].endswith("\""):
        ...
        #   temp = temp[:-1]
          ...
          # new_temp = temp.partition('to')[0]
          # print(new_temp)
          # create_error.no_variable(variable=new_temp[:-1])
        # else:
  ...
  def uppercase(self, body, instance):
    if body[0] in instance.variables.keys():
      newSet = {body[0]:instance.variables[body[0]].upper()}
      instance.variables.update(newSet)
    else:
      create_error.no_variable(variable=body[0])

  def lowercase(self, body, instance):
    if body[0] in instance.variables.keys():
      newSet = {body[0]:instance.variables[body[0]].lower()}
      instance.variables.update(newSet)
    else:
      create_error.no_variable(variable=body[0])